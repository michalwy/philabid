 ...
 */
public class CatalogValueCell extends MultiCurrencyMonetaryAmountCell<Auction> {

    public CatalogValueCell() {
        super();
    ...
    protected void updateItem(MultiCurrencyMonetaryAmount item, boolean empty) {
        ...
        Auction auction = getTableRow() != null ? getTableRow().getItem() : null;

        // Show the icon only if the auction exists and its catalog is inactive
        getIcon().setVisible(
                !empty && auction != null && auction.getCatalogValue() != null && !auction.isCatalogActive());
    }
}